...
global.reload = (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (dir in require.cache) {
      ...
      if (fs.existsSync(dir)) conn.logger.info(`Notei Mudanças no plugin '${filename}'`)
      else {
        conn.logger.warn(`Plugin Deletado '${filename}'`)
        return delete global.plugins[filename]
      }
    } else conn.logger.info(`Exigindo novo plugin '${filename}'`)
...
_quickTest()
  .then(() => conn.logger.info('Teste rápido feito'))
  .catch(console.error)